...
class BenchmarkDataset:
    ...
    def compute_neighbors(self, metric: str = "angular", k: int = 100):
        ...
        if not np.array_equal(self.ground_truth[0, 0:100], validation_vector[0:100]):
            raise Exception(
                "Mismatch between computed neighbors and original ground truth."
            )

    def __str__(self):
        return "BenchmarkDataset(id={}, train={}, test={}, ground_truth={})".format(
            self.id, self.train.shape, self.test.shape, self.ground_truth.shape
        )
