...
class Annoy(SearchIndex):
    """
    This is a this wrapper class to abstract some details from the annoy package.
    """
    ...
    def query(self, vector: np.ndarray, k: int) -> np.ndarray:
        ...
        return np.array(
            self.index.get_nns_by_vector(vector, k, search_k=SEARCH_K), dtype=np.int32
        )
    ...
    def batch_query(self, matrix: np.ndarray, k: int) -> np.ndarray:
        ...
        for i, x in enumerate(matrix):
            neighbors[i] = np.array(
                self.index.get_nns_by_vector(x, k, search_k=SEARCH_K), dtype=np.int32
            )
